...
public class CSVMotioner {
    ...
    public static PyAnalysisResult analyzePeaksFromFile(Context context, String fileName) {
        ...
        if (fileName.contains("SIP_LIPS")){
            try (PyObject pyResult = py.getModule("count_sip_lips")
                    .callAttr("analyze_csv", csvFile.getAbsolutePath())) {
        ...
        //3.鼓臉頰_PUFF_CHEEK
        if (fileName.contains("PUFF_CHEEK")){
            try (PyObject pyResult = py.getModule("count_puff_cheek")
                    .callAttr("analyze_csv", csvFile.getAbsolutePath())) {
        ...
        //4.鼓臉頰_REDUCE_CHEEK
        if (fileName.contains("REDUCE_CHEEK")){
            try (PyObject pyResult = py.getModule("count_reduce_cheek")
                    .callAttr("analyze_csv", csvFile.getAbsolutePath())) {
...
//                    result.debug = dbg;
        ...
        //6.舌頭往右
        if (fileName.contains("TONGUE_RIGHT")){
            try (PyObject pyResult = py.getModule("count_tongue_right")
                    .callAttr("analyze_tongue_csv", csvFile.getAbsolutePath(),"right")) {

                Log.d("CSVMOTIONTEST", "🔥 (TONGUE_RIGHT)Python 回傳: " + pyResult.toString());
...
//
        ...
        //7.舌頭往前、上、下
        if (fileName.contains("TONGUE_FOWARD") || fileName.contains("TONGUE_UP") || fileName.contains("TONGUE_DOWN")) {

            String direction = null;
            if (fileName.contains("TONGUE_FOWARD")) direction = "down"; // 往前我暫時設成 right
            else if (fileName.contains("TONGUE_UP")) direction = "up";
            else if (fileName.contains("TONGUE_DOWN")) direction = "down";

            try (PyObject pyResult = py.getModule("count_tongue_vertical") // ✅ 改成共用的 Python 檔
                    .callAttr("analyze_tongue_csv", csvFile.getAbsolutePath(), direction)) {

                Log.d("CSVMOTIONTEST", "🔥 (TONGUE_" + direction.toUpperCase() + ") Python 回傳: " + pyResult.toString());
