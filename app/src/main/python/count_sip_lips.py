...
def compute_demo_features(t_all, r_all, mask_demo, fs, cutoff, dir_eff,
                          DEMO_SIDE_SEC=2.0, K_DEMO_ENERGY=0.1):
    """
    計算 DEMO 段的能量密度與噪音能量
    Returns: (E_demo, E_noise, energy_thr)
    """
    ...
    # 只取 DEMO 前後各 DEMO_SIDE_SEC 秒這一小窗
    ...
    # 在這一窗裡面重新做濾波 + baseline + detrend
    from scipy.signal import butter, filtfilt
    b, a = butter(4, min(cutoff, 0.49 * fs) / (fs / 2), btype='low')
    r_win_filt = filtfilt(b, a, r_win)
    # 🔥 修正：使用安全的窗口大小
    base_win_size = max(3, min(int(0.09 * fs), len(r_win_filt) // 2))
    base_win = moving_average(r_win_filt, base_win_size)
    ...
    # 噪音能量（DEMO 前後各 DEMO_SIDE_SEC 秒）
    t0 = max(float(t_win[0]), t_demo_start - DEMO_SIDE_SEC)
    t1 = min(float(t_win[-1]), t_demo_end + DEMO_SIDE_SEC)

    mask_left  = (t_win >= t0) & (t_win <  t_demo_start)
    mask_right = (t_win >  t_demo_end) & (t_win <= t1)

    if not (np.any(mask_left) and np.any(mask_right)):
        E_noise = 0.0
    else:
        mask_noise = mask_left | mask_right
        t_noise    = t_win[mask_noise]
        r_noise    = r_win_det[mask_noise]
        E_noise = energy_density_interval_dir(
            r_noise, t_noise, fs,
            t_noise[0], t_noise[-1], dir_eff
        )

    # 能量門檻（先不考慮抖動，所以 E_noise=0）
    E_noise = 0
    energy_thr = max(K_DEMO_ENERGY * E_demo, E_noise)

    return float(E_demo), float(E_noise), float(energy_thr)
...
def analyze_csv(file_path: str, cutoff: float = 0.8, order: int = 4,
    ...
    try:
        ...
        # 濾波
        cutoff_final, segments_cutoff = auto_cutoff_from_signal(t, r, fs)
        r_filt_mixed = np.zeros_like(r)

        for seg in segments_cutoff:
            m_seg = (t >= seg["start"]) & (t <= seg["end"])
            # 🔥 修正：改用更寬鬆的門檻（3個點就好）
            if np.sum(m_seg) < 3:
                continue
            r_seg = r[m_seg]
            r_filt_seg = lowpass_filter(r_seg, fs=fs, cutoff=seg["cutoff"], order=order)
            r_filt_mixed[m_seg] = r_filt_seg
        ...
        # 🔥 修正：基線扣除加入安全檢查
        baseline_window = max(3, min(int(4.0 * fs), len(r_filt) // 2))
        print(f"🔧 Baseline 窗口 = {baseline_window} samples (FS={fs}Hz)")
        baseline = moving_average(r_filt, baseline_window)
        ...
        # 計算 DEMO 能量特徵
        demo_E, demo_E_noise, demo_E_thr = compute_demo_features(
            t_all_full, r_all_full, mask_demo, fs, cutoff_final, dir_eff,
            DEMO_SIDE_SEC, K_DEMO_ENERGY
        )

        energy_threshold = alpha * demo_E_thr
        print(f"🔥 能量門檻 = {energy_threshold:.2e} (α={alpha}, E_thr={demo_E_thr:.2e})")
        ...
        return {
            ...
            "debug": {
                ...
                "min_interval": int(0.2 * fs),
                "demo_E": demo_E,
                "demo_E_noise": demo_E_noise,
                "demo_E_thr": demo_E_thr,
                "energy_threshold": energy_threshold,
            }
        }
...
if __name__ == "__main__":
    ...
    CSV_PATH = r"C:\Users\plus1\Downloads\1118從S24取出資料\FaceTraining_SIP_LIPS_20251104_035124.csv" # 8/9
    ...
    CSV_PATH = r"C:\Users\plus1\Downloads\1118從S24取出資料\FaceTraining_SIP_LIPS_20251102_190714.csv" # 8/10
